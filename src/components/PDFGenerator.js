...
export async function generatePDF(members, entries, currentWeekStart, weekDays, trackerType = 'pineal') {
  ...
  const accentColor = trackerType === 'tiger' ? '#ff1493' : trackerType === 'lucky' ? '#ff0000' : '#d4a017';
  ...
  doc.setFontSize(28);
  doc.text(trackerType === 'tiger' ? 'TIGER LOG' : trackerType === 'lucky' ? 'LUCKY LOG' : 'PINEALLOG', 14, 24);
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  const generatorName = trackerType === 'tiger' ? 'Tiger Log' : trackerType === 'lucky' ? 'Lucky Log' : 'PinealLog';
  doc.text(`Generated by ${generatorName} · ${new Date().toLocaleString()}`, 14, 200);

  // Save
  const fileNamePrefix = trackerType === 'tiger' ? 'TigerLog' : trackerType === 'lucky' ? 'LuckyLog' : 'PinealLog';
